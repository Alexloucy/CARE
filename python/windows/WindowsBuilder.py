...
class WindowsFolderBuilder:
    ...
    URLS_PYTHON = {
        ...
        '3.10.11': r'https://www.python.org/ftp/python/3.10.11/python-3.10.11-embed-amd64.zip',
        '3.11.3': r'https://www.python.org/ftp/python/3.11.3/python-3.11.3-embed-amd64.zip',
    }
...
def build_care_windows(release_dir, cache_dir, python_ver: str = '3.11.3', backend: str = 'cuda'):
    """
    Build a portable Windows runtime for the CARE Python project using the
    same embedded-Python approach as DeepFaceLive.
    """
    builder = WindowsFolderBuilder(
    ...
    # Path to the source Python project (the `python` folder in the repo).
    project_python_path = Path(__file__).resolve().parents[1]
    requirements_win32 = project_python_path / 'requirements-win32.txt'

    # Install PyTorch with CUDA wheels if requested.
    if backend == 'cuda':
        # Match the development environment: CUDA 12.8 wheels for Python 3.11.
        builder.install_pip_package(
            'torch==2.9.1+cu128 torchvision==0.24.1+cu128 torchaudio==2.9.1+cu128 '
            '--index-url https://download.pytorch.org/whl/cu128'
        )

    # Install the rest of the Python dependencies for inference.
    # Create a filtered requirements file for the embedded runtime.
    if requirements_win32.exists():
        runtime_reqs = builder.python_path / 'requirements-care-win32.txt'

        # Read requirements file with tolerant encoding handling.
        data = requirements_win32.read_bytes()
        text = None
        ...
        runtime_reqs.write_text('\n'.join(filtered_lines), encoding='utf-8')

        # Use --no-build-isolation so that packages with pyproject.toml (e.g. clipboard, pyqtree)
        # can build against the already-installed setuptools in this embedded environment.
        builder.install_pip_package(f'--no-build-isolation -r "{runtime_reqs}"')
    else:
        print(f'WARNING: {requirements_win32} not found, skipping requirements installation.')

    # Move CUDA DLLs from torch into the shared CUDA bin folder so that
    # they are available on machines without a local CUDA toolkit.
    if backend == 'cuda':
        ...
        if torch_lib_path.exists():
            ...
            print(f'WARNING: torch lib path not found at {torch_lib_path}, skipping CUDA DLL move.')

    # Copy the CARE Python project (sources + models) into the embedded runtime.
    care_internal_path = builder.get_internal_path() / 'care'
    care_internal_path.mkdir(parents=True, exist_ok=True)

    # Copy selected subdirectories.
    for dirname in ('config', 'dinov3', 'models'):
        src_dir = project_python_path / dirname
        dst_dir = care_internal_path / dirname
        if src_dir.exists():
    ...
    # Copy top-level Python modules used by the CARE runtime.
    for py_file in project_python_path.glob('*.py'):
        dst_file = care_internal_path / py_file.name
        print(f'Copying {py_file} -> {dst_file}')
        shutil.copy(py_file, dst_file)

    # Optional helper to run CARE directly via a .bat file.
    # Forward all user-supplied arguments to main.py.
    builder.create_run_python_script('run-care.bat', 'care\\main.py', '%*')

    # Clean up temporary state and __pycache__ directories.
    builder.cleanup()
...
if __name__ == '__main__':
    ...
    p.add_argument('--build-type', required=True, choices=['dfl-windows', 'care-windows'])
    ...
    # DeepFaceLive was built against 3.8.x; CARE uses 3.11.x.
    if args.build_type == 'care-windows' and args.python_ver == '3.8.10':
        args.python_ver = '3.11.3'
    ...
    elif args.build_type == 'care-windows':
        build_care_windows(release_dir=args.release_dir,
                           cache_dir=args.cache_dir,
                           python_ver=args.python_ver,
                           backend=args.backend)
