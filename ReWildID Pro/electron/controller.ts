 ...
 */
export async function smartReID(
    imageIds: number[],
    species: string,
    stream: (txt: string) => void
): Promise<{ ok: boolean; reidRunId?: number; error?: string }> {
    ...
    try {
        ...
        if (imagesWithoutDetections.length > 0) {
            ...
            // Get the image paths
            const imagesToDetect = DatabaseService.getImagesByIds(imagesWithoutDetections);
        ...
        const matchingDetections = allDetections.filter(
            det => det.label?.toLowerCase() === speciesLower
        );
        ...
        const inputJsonPath = path.join(tempDir, `reid_input_${Date.now()}.json`);
        const outputJsonPath = path.join(tempDir, `reid_output_${Date.now()}.json`);
        ...
        // Cleanup temp files
        await fs.remove(inputJsonPath);
        await fs.remove(outputJsonPath);
